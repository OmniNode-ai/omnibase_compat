...
(OMN-14669, WS7 fan-out #6 of the OMN-14655 canary; ported byte-for-byte from
omnibase_infra#2318 / omnimarket#1783 / omniclaude#1904 so every surface
enforces the identical rule).

DRIFT-2 (false-green hook) recurrence guard. The canary repos had gate wrappers
...
Note: this prose deliberately avoids writing the detected token pair (a literal
zero-exit adjacent to a missing-path phrase) so the meta-gate does not flag its
own docstring when a hook `entry:` resolves this file directly (omnibase_compat
invokes it via bare `uv run python`, so `_resolve_script` scans this module).
...
_SKIP_ENV_VAR_RE = re.compile(r"\$\{?\b\w*(SKIP|BYPASS)\w*(?<!PATTERN)(?<!REGEX)\b", re.IGNORECASE)
...
def _scan_text(source: str, label: str) -> list[str]:
    ...
    for idx, line in enumerate(lines):
        ...
        if any(_SKIP_ENV_VAR_RE.search(w) and not _is_suppressed(w) for w in window) and any(
            "if" in w or "&&" in w for w in window
        ):
            violations.append(
...
def _resolve_script(entry: str) -> Path | None:
    ...
    for token in entry.replace("\n", " ").split():
        if token.endswith((".sh", ".py")) and token.startswith(("scripts/", ".pre-commit-hooks/")):
