...
from omnibase_compat.contracts.evidence import (
...
def test_contract_evidence_spec_accepts_artifact_first_proof() -> None:
    spec = ModelContractEvidenceSpec(
        ticket_id="OMN-11261",
        summary="Decouple contract evidence from PR-number identity.",
        artifact_names=("ModelContractEvidenceSpec",),
        repository_surfaces=("onex_change_control/contracts",),
        acceptance_criteria=("stable proof validates a model import",),
        stable_proofs=(
            ModelContractEvidenceProof(
                proof_id="model-import",
                proof_kind="model_import",
                description="Model imports from compat during migration.",
                target="omnibase_compat.contracts.evidence.ModelContractEvidenceSpec",
                model_path="omnibase_compat.contracts.evidence.ModelContractEvidenceSpec",
            ),
        ),
        provenance=(
            ModelEvidenceProvenance(
                repo="omnibase_compat",
                branch="jonah/omn-11261-compat-contract-evidence-model",
                pr_number=92,
            ),
        ),
    ...
    assert spec.provenance[0].pr_number == 92
...
def test_stable_proof_allows_non_pr_github_artifact_url() -> None:
    proof = ModelContractEvidenceProof(
        ...
        target="https://github.com/OmniNode-ai/omnibase_compat/blob/main/pyproject.toml",
        command="test -n https://github.com/OmniNode-ai/omnibase_compat/blob/main/pyproject.toml",
    )
