...
"""ModelRuntimeDeploymentProof — runtime deployment proof wire DTO.
...
onex.evt.omnimarket.runtime-deployment-proof.v1). OCC owns the source of truth;
compat is temporary (OMN-12576). The proof is assembled from the live runtime
attestation surfaces plus the per-lane health/ready/digest probe.
"""
...
class ModelRuntimeDeploymentProof(BaseModel):
    ...
    runtime_lane: EnumRuntimeLane = Field(..., description="Runtime lane that was probed.")
    source_sha: str = Field(
        ..., min_length=1, description="Source commit SHA bound to the deployed artifact."
    )
    ...
    health_status: ProbeStatus = Field(..., description="Per-lane /health probe result.")
    ...
    topology_manifest_sha256: str | None = Field(
        default=None, description="Topology manifest hash from the runtime manifest reducer."
    )
    package_versions: dict[str, str] = Field(
        default_factory=dict, description="Deployed package versions from runtime attestation."
    )
    runtime_source_hash: str | None = Field(
        default=None, description="Runtime source hash from the runtime source attestor."
    )
...
__all__: list[str] = ["ModelRuntimeDeploymentProof"]
