...
renderer/parser lives alongside it in ``pr_occ_metadata_stamp`` / OMN-14188).

Relocated from ``omnibase_core.models.validation`` to ``omnibase_compat`` under
OMN-14223 (operator placement correction to OMN-14180).
"""
...
class ModelPrOccMetadataStamp(BaseModel):
    ...
    def _normalize_tickets(cls, value: object) -> tuple[str, ...]:
        ...
        for raw in value:
            if not isinstance(raw, str):
                raise ValueError(f"evidence ticket must be a string, got {type(raw).__name__}")
            token = raw.strip().upper()
            if not _TICKET_RE.fullmatch(token):
                raise ValueError(f"evidence ticket {raw!r} does not match OMN-<n> (e.g. OMN-14187)")
    ...
    def as_dict(self) -> dict[str, object]:
        return {
            ...
            "evidence_source": (
                self.evidence_source.as_dict() if self.evidence_source is not None else None
            ),
