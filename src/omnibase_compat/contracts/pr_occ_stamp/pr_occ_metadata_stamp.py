...
Relocated from ``omnibase_core.validation`` to ``omnibase_compat`` under
...
def render_pr_occ_metadata_stamp(stamp: ModelPrOccMetadataStamp) -> str:
    ...
    human_body = "".join(
        section.content for section in stamp.body_sections if not section.is_stamp_section
    )
...
def _bind_allowlist_to_latest_token(
    ...
    rebound = list(tokens)
    latest = rebound[-1]
    rebound[-1] = latest.model_copy(update={"allowlist_receipt_id": allowlist_receipt_id})
    return rebound
