...
class ModelWorkerContract(BaseModel):
    ...
    heartbeat_interval_seconds: int = Field(
        default=300,
        gt=0,
        description="Max silence (no TaskUpdate or SendMessage) before stall_action fires.",
    )
...
def load_worker_contract(data: dict[str, Any]) -> ModelWorkerContract:
    ...
    if not isinstance(data, dict):
        raise TypeError(f"worker contract data must be a mapping, got {type(data).__name__}")
