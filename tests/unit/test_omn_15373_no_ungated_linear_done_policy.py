...
* required no closing keyword — bare branch-name extraction was the ONLY signal
  in this repo's copy, which never received the OMN-14641 hardening that added a
  ``Closes``-word fallback and an ``evidence/*`` / ``occ/*`` companion-branch
  skip elsewhere;
...
writer on every merge to ``main``, ``develop`` or ``dev``.
...
The conjunction is deliberate. Signal 1 alone is legitimate — a sanctioned
evidence-gated caller issues exactly that mutation and resolves states by NAME,
...
It lives under ``tests/`` — alongside the existing repo-scanning policy gate
``tests/unit/test_no_infra_edge.py`` — because this repo's change-aware selector
falls back to ``["tests/"]`` for any change with no ``src/omnibase_compat/``
mapping, which is exactly the shape of a workflow re-add
...
# defect written in Python. ``.pre-commit-hooks`` is included because it holds
# committed shell recipes that run on every commit.
_SCAN_DIRS: tuple[str, ...] = (".github", ".pre-commit-hooks", "scripts", "src")
...
_SELF_EXEMPT: frozenset[str] = frozenset(
    {"tests/unit/test_omn_15373_no_ungated_linear_done_policy.py"}
)
...
# (omnibase_compat@dev blob 9984cbcd92ecb94ffa02eee09a22473b8e82242d).
...
def test_by_name_state_resolution_is_not_flagged() -> None:
    ...
    — omnibase_compat has NO ``issueUpdate`` call site at all outside the
    workflow this PR deletes (verified at ``omnibase_compat@dev`` f636d2b2:
