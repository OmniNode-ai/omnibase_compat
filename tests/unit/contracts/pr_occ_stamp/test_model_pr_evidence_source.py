...
class TestModelPrEvidenceSource:
    def test_valid_occ_pr_variant(self) -> None:
        src = ModelPrEvidenceSource(kind=EnumPrEvidenceSourceKind.OCC_PR, occ_pr_number=123)
    ...
    def test_invalid_commit_sha_pattern_raises(self, bad_sha: str) -> None:
        with pytest.raises(ValidationError):
            ModelPrEvidenceSource(kind=EnumPrEvidenceSourceKind.COMMIT_SHA, commit_sha=bad_sha)
    ...
    def test_roundtrip_model_dump(self) -> None:
        src = ModelPrEvidenceSource(kind=EnumPrEvidenceSourceKind.OCC_PR, occ_pr_number=999)
    ...
    def test_frozen_attribute_mutation_raises(self) -> None:
        src = ModelPrEvidenceSource(kind=EnumPrEvidenceSourceKind.OCC_PR, occ_pr_number=1)
        with pytest.raises(ValidationError):
    ...
    def test_to_json_deterministic(self) -> None:
        src = ModelPrEvidenceSource(kind=EnumPrEvidenceSourceKind.OCC_PR, occ_pr_number=42)
