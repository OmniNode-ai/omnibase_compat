...
def _is_foreign_root_checkout(step: dict[str, Any]) -> str | None:
    ...
    """
    uses = step.get("uses")
    if not isinstance(uses, str) or not uses.split("@", 1)[0].endswith("actions/checkout"):
        return None
    ...
    for workflow_path in workflow_paths:
        ...
        for job_id, job in jobs.items():
            ...
            for step in steps:
                ...
                working_directory = _run_working_directory(step, job_default, workflow_default)
                if (
                ...
                for match in _SCRIPT_TOKEN_RE.finditer(_strip_full_line_comments(run_body)):
                    references.append(
                        ScriptReference(
...
def find_missing(references: list[ScriptReference], repo_root: Path) -> list[ScriptReference]:
...
def test_every_workflow_referenced_local_script_exists() -> None:
    ...
    assert not missing, (
        ...
        + "\n".join(f"  - {ref.workflow} (job {ref.job}) runs {ref.path}" for ref in missing)
...
""",
    ...
    references, skipped = collect_script_references(tmp_path)
    assert [ref.path for ref in references] == ["scripts/ci/occ_manual_replay_precheck.py"]
    assert skipped == []
...
def test_find_missing_reports_absent_targets(tmp_path: Path) -> None:
    ...
    assert [ref.path for ref in find_missing(references, tmp_path)] == ["scripts/ci/absent.py"]
...
def _main() -> int:
    ...
    if missing:
        ...
        for ref in missing:
            print(f"  - {ref.workflow} (job {ref.job}) runs {ref.path}", file=sys.stderr)
        return 1
